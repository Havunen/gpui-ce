...
pub fn derive_refineable(input: TokenStream) -> TokenStream {
        ...
        .map(|f| {
            if derives_serialize {
                if is_refineable_field(f) {
                    quote! { #[serde(default, skip_serializing_if = "::gpui::IsEmpty::is_empty")] }
                } else {
    ...
    let refineable_refine_assignments: Vec<TokenStream2> = fields
        ...
        .map(|field| {
            ...
            if is_refineable {
                quote! {
                    ::gpui::Refineable::refine(&mut self.#name, &refinement.#name);
                }
            } else if is_optional {
                quote! {
    ...
    let refineable_refined_assignments: Vec<TokenStream2> = fields
        ...
        .map(|field| {
            ...
            if is_refineable {
                quote! {
                    self.#name = ::gpui::Refineable::refined(self.#name, refinement.#name);
                }
            } else if is_optional {
                quote! {
                    if let Some(value) = refinement.#name {
    ...
    let refinement_is_empty_conditions: Vec<TokenStream2> = fields
        ...
        .map(|(i, field)| {
            ...
            let condition = if is_refineable_field(field) {
                quote! { ::gpui::IsEmpty::is_empty(&self.#name) }
            } else {
    ...
    let refineable_is_superset_conditions: Vec<TokenStream2> = fields
        ...
        .map(|field| {
            ...
            if is_refineable {
                quote! {
                    if !::gpui::Refineable::is_superset_of(&self.#name, &refinement.#name) {
                        return false;
                    }
                }
            } else if is_optional {
    ...
    let refineable_subtract_assignments: Vec<TokenStream2> = fields
        ...
        .map(|field| {
            ...
            if is_refineable {
                quote! {
                    #name: ::gpui::Refineable::subtract(&self.#name, &refinement.#name),
                }
            } else if is_optional {
                quote! {
    ...
    let r#gen = quote! {
        ...
        impl #impl_generics ::gpui::Refineable for #ident #ty_generics
            #where_clause
        {
        ...
        impl #impl_generics ::gpui::Refineable for #refinement_ident #ty_generics
            #where_clause
        {
            type Refinement = #refinement_ident #ty_generics;
        ...
        impl #impl_generics ::gpui::IsEmpty for #refinement_ident #ty_generics
            #where_clause
        {
